...
!(async () => {
    ...
    for (let i in cookiesArr) {
        ...
        if(kois.indexOf(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])!=-1){
            ...
            // openred.push({id: i, cookie: cookie})
        }
...
function openredf(cook){
    ...
    //延迟
    requestHBApi('h5receiveRedpacketAll',cook,{
        "random": num,
        "log":"42588613~8,~0iuxyee",
        "sceneid":"JLHBhPageh5"
    })
...
function requestHBApi(functionId, cookie, body = {}) {
    return new Promise(resolve => {
        $.post({
            url: `${JD_API_HOST}?appid=jinlihongbao&functionId=${functionId}&loginType=2&client=jinlihongbao&clientVersion=10.2.2&osVersion=-1`,
            headers: {
                "Cookie": cookie,
