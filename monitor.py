...
async def outfile(filename, context, iscover):
    """
    ...
    if record == False:
        ...
        try:
            if iscover == False:
                with open((self.pwd + '/log/{0}'.format(filename)), 'a+', encoding='utf-8') as f1:
                    f1.write(f'{context}\n')
            else:
                if iscover == True:
                    with open((self.pwd + '/{0}'.format(filename)), 'w+', encoding='utf-8') as f1:
                        f1.write(f'{context}')
        except Exception as e:
            try:
                print(e)
